...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
	...
	while True:
		...
		if np.max(np.fabs(value_function - prev_value_function)) < tol:
			break
		else:
			i += 1
	...
	for state in range(nS):
		...
		for action in range(nA):
			for next_sr in P[state][action]:
				...
				q_value[action] += prob * (reward + gamma * value_from_policy[next_state])
	...
	while True:
		...
		for state in range(nS):
			...
			for action in range(nA):
				for next_sr in P[state][action]:
					prob, next_state, reward, done = next_sr
					q_values[action] += prob * (reward + gamma * prev_value_function[next_state])
